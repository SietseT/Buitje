...
- `apps/backend` — Fastify + TypeScript (Node, ESM). Polls the KNMI Open Data API every 5 minutes, parses the HDF5 radar composite, reprojects it, colorizes it to PNG, and serves it.
...
- **The shared anonymous KNMI API key is rate-limited to 50 req/min across ALL anonymous users**, not just this app — expect occasional 429s under normal use. `apps/backend/src/knmi/client.ts` retries with backoff (respects `Retry-After`), and the startup backfill (`poller.ts`) deliberately spaces requests ~1.5s apart. Don't remove that spacing or the retry logic without rethinking the rate limit budget (each frame costs 2 requests: list + download-url).
- **Backend backfills history on startup** (fetches the last `MAX_FRAMES` files, not just the latest) so the timeline isn't empty for 2 hours after every restart. This means restarting the backend in dev re-runs a ~30-60s backfill each time (`tsx watch` will do this on every source save).
